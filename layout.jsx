    ...
    <>
      ...
      <header className="header">
        <div className="container header-inner header-grid">
          ...
          <div className="header-salah">
            <div className="header-salah-date">
              <b>27 May 2026</b> <span>·</span> <span>10 Dhū al-Hijjah 1447</span>
            </div>
            <div className="header-salah-row">
              <a className="header-salah-link"
              onClick={(e) => {e.preventDefault();setRoute("prayer");}}
              href="#">Full Timetable<br />& Calendar</a>
              <div className="header-salah-table">
                <span></span>
                <span className="header-salah-pname">Fajr</span>
                <span className="header-salah-pname">Zuhr</span>
                <span className="header-salah-pname header-salah-pname-next">ʻAsr</span>
                <span className="header-salah-pname">Maghrib</span>
                <span className="header-salah-pname">ʻIshā</span>

                <span className="header-salah-rowlabel">Begins</span>
                <span className="tabnum header-salah-cell passed">{TODAY_TIMES[activeMasjid].fajr[0]}</span>
                <span className="tabnum header-salah-cell passed">{TODAY_TIMES[activeMasjid].dhuhr[0]}</span>
                <span className="tabnum header-salah-cell header-salah-next">{TODAY_TIMES[activeMasjid].asr[0]}</span>
                <span className="tabnum header-salah-cell">{TODAY_TIMES[activeMasjid].maghrib[0]}</span>
                <span className="tabnum header-salah-cell">{TODAY_TIMES[activeMasjid].isha[0]}</span>

                <span className="header-salah-rowlabel">Jamāʿah</span>
                <span className="tabnum header-salah-cell jamaah passed">{TODAY_TIMES[activeMasjid].fajr[1]}</span>
                <span className="tabnum header-salah-cell jamaah passed">{TODAY_TIMES[activeMasjid].dhuhr[1]}</span>
                <span className="tabnum header-salah-cell jamaah header-salah-next">{TODAY_TIMES[activeMasjid].asr[1]}</span>
                <span className="tabnum header-salah-cell jamaah">{TODAY_TIMES[activeMasjid].maghrib[1]}</span>
                <span className="tabnum header-salah-cell jamaah">{TODAY_TIMES[activeMasjid].isha[1]}</span>
              </div>
              <div className={`masjid-picker header-salah-picker ${openMenu === "masjid" ? "open" : ""}`}>
                <button className="masjid-picker-trigger" onClick={(e) => {e.stopPropagation();setOpenMenu(openMenu === "masjid" ? null : "masjid");}}>
                  <Icon.Pin width="13" height="13" style={{ color: "var(--gold-600)", flexShrink: 0 }} />
                  {activeMasjidObj.short}
                  <svg className="masjid-picker-trigger-chev" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6" /></svg>
                </button>
                <div className="masjid-picker-menu">
                  {MASJIDS.map((m) =>
                  <button key={m.id}
                  className={`masjid-picker-item ${activeMasjid === m.id ? "active" : ""}`}
                  onClick={() => {setActiveMasjid(m.id);setOpenMenu(null);}}>
          ...
          <div className="nav-end">
            <div className={`masjid-picker masjid-picker-mobile ${openMobile ? "open" : ""}`} ref={mobileRef}>
              <button className="masjid-picker-trigger" onClick={() => setOpenMobile(!openMobile)}>
                <Icon.Pin width="13" height="13" style={{ color: "var(--gold-600)", flexShrink: 0 }} />
                {activeMasjidObj.short}
                <svg className="masjid-picker-trigger-chev" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6" /></svg>
              </button>
              <div className="masjid-picker-menu">
                ...
                <button key={m.id}
                className={`masjid-picker-item ${activeMasjid === m.id ? "active" : ""}`}
                onClick={() => {setActiveMasjid(m.id);setOpenMobile(false);}}>
                    <span>{m.name}<br /><small>{m.area}</small></span>
                    {activeMasjid === m.id &&
                  <span className="masjid-picker-check">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
            ...
            <div className="header-socials">
              <a href="#" aria-label="Facebook" className="header-social"><Icon.Facebook width="14" height="14" /></a>
              <a href="#" aria-label="Instagram" className="header-social"><Icon.Instagram width="14" height="14" /></a>
              <a href="#" aria-label="YouTube" className="header-social"><Icon.Youtube width="14" height="14" /></a>
              <a href="#" aria-label="Twitter" className="header-social"><Icon.Twitter width="14" height="14" /></a>
            </div>
          </div>
        </div>
      </header>
      <SalahStrip activeMasjid={activeMasjid} setActiveMasjid={setActiveMasjid} />
    </>);
